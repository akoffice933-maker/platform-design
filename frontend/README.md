...
## ИИ через OpenRouter (опция, docs/12)

В правой колонке сессии — панель «ИИ · OpenRouter»: ключ (BYOK, живёт в localStorage),
выбор бесплатной модели (дефолт `openrouter/free`), тумблеры. Две функции:
**ИИ-судья** свободных ответов (input-узлы, валидация + гарантированный детерминированный
фолбэк) и **варианты реплик ИИ-клиента** (кнопка под репликами `text`; критический эпизод
модели не отдаётся). Без ключа сессия полностью работает — ИИ не зависимость.
