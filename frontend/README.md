...
**Генератор сценариев**: Библиотека → «✨ Сгенерировать» — тема запроса, фокус-навык,
сложность → полный сценарий с ветвлениями и критическим эпизодом (браузерный валидатор
графа перед запуском; без ключа — 3 шаблонные темы). **Свободные ответы психолога** на
выборах: ИИ-судья ведёт по ближайшей ветке графа и оценивает по навыкам (фолбэк — по
словоформам). **ИИ-судья** свободных ответов (input-узлы) и **варианты реплик клиента**
(кнопка ⟳ под репликами; критический эпизод модели не отдаётся). Все ошибки видимы:
кнопка «Проверить ключ», статус каждой попытки, подпись источника оценки.
